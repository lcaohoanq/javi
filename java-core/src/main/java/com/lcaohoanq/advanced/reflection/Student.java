...
class SortUtils {
     ...
     */
    public static <T> void sort(List<? extends T> list, List<SortCriterion> criteria, Class<T> clazz) {
        if (list == null || list.isEmpty() || criteria == null || criteria.isEmpty()) {
            return;
        }
        ((List<T>) list).sort(createComparator(criteria, clazz));
    }
     ...
     * @param <T>       Type of objects in the list
     */
    public static <T> void sort(List<? extends T> list, String fieldName, SortOrder order, Class<T> clazz) {
        sort(list, List.of(new SortCriterion(fieldName, order)), clazz);
    }

    private static <T> Comparator<T> createComparator(List<SortCriterion> sortCriteria, Class<T> clazz) {
        return (o1, o2) -> {
            try {
    ...
    private static <T extends Comparable<T>> int compareValues(Object value1, Object value2, SortOrder order) {
        // Handle null values
        if (value1 == null && value2 == null) return 0;
        if (value1 == null) return order == SortOrder.ASC ? -1 : 1;
        if (value2 == null) return order == SortOrder.ASC ? 1 : -1;
